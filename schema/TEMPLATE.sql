...
SELECT 'A MENSAGEM, O ENVELOPE E A ARTE.';
SELECT 'EU SOU UM ENVELOPE';
SELECT 'COMO ATRAIR PESSOAS PARA PEGAREM O ENVELOPE? R= COM A ARTE.';
...
/*3) Crie a instrução SQL para alterar o nome da
...
qtd_estoque_pro = 9,
qtd_minima_pro = 6   WHERE codigo_pro = 11;
...
/*8) Apresentar todos os dados dos produtos que não
...
/*10) Apresentar o total de variedades de Produtos
cadastrados. */
SELECT COUNT(DISTINCT codigo_pro) FROM tabprodutos;

/*11) Apresentar a quantidade total de Produtos
cadastrados. */
SELECT COUNT(codigo_pro) FROM tabprodutos;

/*12) Apresentar o valor total dos Produtos em estoque. */
SELECT SUM(qtd_estoque_pro*preco_custo_pro)AS TOTAL_VALOR_CUSTO, 
SUM(qtd_estoque_pro*preco_venda_pro)AS TOTAL_VALOR_VENDAS FROM tabprodutos;
